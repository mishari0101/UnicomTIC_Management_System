...
namespace UnicomTICManagementSystem.Views
{
    public partial class UserForm : Form
    {
        ...
        {
            // We also check if the password is still the placeholder text.
            ...
            {
                MessageBox.Show("Username and a valid Password cannot be empty.", "Error");
                return;
            }
            _controller.AddUser(txtUsername.Text, txtPassword.Text, cmbRole.SelectedItem.ToString());
            MessageBox.Show("User added successfully!");
            LoadUsersGrid();
            ResetPasswordBox(); // Reset after adding.
        }

        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (dgvUsers.CurrentRow == null) return;
            // Also check for the placeholder text here.
            if (string.IsNullOrWhiteSpace(txtUsername.Text) || string.IsNullOrWhiteSpace(txtPassword.Text) || txtPassword.Text == passwordPlaceholder)
            {
                MessageBox.Show("Username and a new Password are required for an update.", "Error");
                return;
            }
