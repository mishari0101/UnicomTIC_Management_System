...
namespace UnicomTICManagementSystem.Views
{
    public partial class CourseForm : Form
    {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                // FIX: Use Convert.ToInt32 for a safer conversion from the cell value to an integer.
                int courseId = Convert.ToInt32(dgvCourses.CurrentRow.Cells["CourseID"].Value);
                string newName = txtCourseName.Text;

                if (!string.IsNullOrWhiteSpace(newName))
                {
                    _courseController.UpdateCourse(courseId, newName);
                ...
                {
                    MessageBox.Show("Please enter a new course name.", "Error");
                }
            }
            else
            {
